...
describe('Phase 1 Specs - Tree', () => {
  ...
  context('Invalid Data', () => {
    it('does not allow `tree` attribute to be empty', async () => {
      try {
        await runSQL("INSERT INTO 'Trees' DEFAULT VALUES;", SERVER_DB_TEST_FILE)
      } catch(err) {
        try {
  ...
  context('Valid Data', () => {
    ...
    it('allows a number greater than 0 for the `heightFt` and `groundCircumferenceFt` attribute values', async () => {
      await models.Tree.create({ tree: 'avocado', heightFt: 100, groundCircumferenceFt: 0.1 });
    });

    it('uses the current time as default values for `createdAt` and `updatedAt` attribute values', async () => {
      await runSQL("INSERT INTO 'Trees' (tree) VALUES ('apple');", SERVER_DB_TEST_FILE);
      const testTree = await models.Tree.findOne({ where: { tree: 'apple' }, attributes: ['createdAt', 'updatedAt']});
